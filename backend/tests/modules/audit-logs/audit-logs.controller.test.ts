...
const { getAuditLogs } =
  await import('../../../src/modules/audit-logs/audit-logs.controller.js')
...
describe('audit-logs.controller', () => {
  ...
  it('accepts the full writable audit entity ID length', async () => {
    listAuditLogs.mockResolvedValueOnce({ data: [], nextOffset: null } as never)
    const entityId = 'e'.repeat(160)

    await getAuditLogs(request({ entityId }), response())

    expect(listAuditLogs).toHaveBeenCalledWith(expect.objectContaining({ entityId }))
  })
})
