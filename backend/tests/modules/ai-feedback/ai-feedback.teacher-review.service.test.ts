...
describe('AI writing feedback teacher review service', () => {
  ...
  it('rejects an AI decision when replacement tombstones its reviewed draft', async () => {
    prisma.aiFeedbackDraft.findFirst.mockResolvedValueOnce(baseDraft as never)
    prisma.aiFeedbackDraft.updateMany.mockResolvedValueOnce({ count: 0 })
    await expect(
      approveAiWritingFeedbackDraft(
        { submissionId, draftId },
        { feedbackMd: 'Old review' },
        teacherActor,
      ),
    ).rejects.toMatchObject({ statusCode: 409 })
    expect(prisma.$queryRaw.mock.calls[0][0].sql).toContain('FROM submissions')
    expect(prisma.aiFeedbackDraft.updateMany).toHaveBeenCalledWith(
      expect.objectContaining({
        where: expect.objectContaining({ id: draftId, deletedAt: null }),
      }),
    )
