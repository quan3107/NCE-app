...
async function main(): Promise<void> {
  ...
  const courseSeeds: Array<{
  ...
  }> = [
    {
      ...
      description: 'Four-week intensive on Task 1 and Task 2 academic writing responses.',
      learningOutcomes: [
  ...
  const courseByTitle = new Map(createdCourses.map((course) => [course.title, course]))
  ...
  const notificationSeeds: Array<{
    ...
    },
    {
      ...
      payload: {
        ...
        startTime: courseByTitle.get('IELTS Listening Lab')?.createdAt ?? new Date(),
      },
    },
    {
    ...
    notificationSeeds.map((seed) => {
      ...
      return prisma.notification.create({
        data: {
          ...
          status: seed.status,
          sentAt: seed.sentOffsetDays !== null ? daysFromNow(seed.sentOffsetDays) : null,
          readAt:
  ...
  for (const seed of fileSubmissionSeeds) {
    ...
    if (
      seed.graderEmail &&
      seed.rawScore !== undefined &&
      seed.finalScore !== undefined
    ) {
      const grader = userByEmail.get(seed.graderEmail)
      if (!grader) {
  ...
  await Promise.all(
    sessionSeeds.map((seed, index) => {
      ...
      const ipHash = seed.ip ? createHash('sha256').update(seed.ip).digest('hex') : null
