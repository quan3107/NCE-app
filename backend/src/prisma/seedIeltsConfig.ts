...
const assignmentTypes = buildAssignmentTypes(ieltsAssignmentTypeRows)
...
const sampleTimingOptions = buildSampleTimingOptions([
  ...
  ['specific_date', 'On a specific date', 'Show on a specific date and time', 4],
])
...
async function ensureConfigVersion(prisma: Prisma.TransactionClient): Promise<void> {
...
async function seedConfigTables(prisma: Prisma.TransactionClient): Promise<void> {
