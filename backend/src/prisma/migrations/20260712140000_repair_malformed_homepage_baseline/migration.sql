-- File: backend/src/prisma/migrations/20260712140000_repair_malformed_homepage_baseline/migration.sql
-- Purpose: Repair malformed homepage baselines without accepting NULL validation results.
-- Why: PostgreSQL BOOL_AND ignores NULL, so every candidate predicate must fail closed.
...
WITH invalid_baselines AS (
  ...
  WHERE page.page_key = 'homepage'
    ...
    AND NOT COALESCE(
      CASE
        WHEN JSONB_TYPEOF(baseline.content_json -> 'stats') = 'array' THEN (
          SELECT
            COUNT(*) = 3
          ...
          FROM JSONB_ARRAY_ELEMENTS(baseline.content_json -> 'stats') stat
        )
        ELSE FALSE
      END,
      FALSE
