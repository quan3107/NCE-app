...
import {
  buildAnalyticsCourseWhere,
  buildAnalyticsSubmissionDateWhere,
...
export async function getTeacherAnalytics(
  ...
  const courses: CourseRecord[] = await prisma.course.findMany({
    where: buildAnalyticsCourseWhere(user, filters),
    select: {
      id: true,
      title: true,
    },
    ...
    assignmentIds.length
      ? await prisma.submission.findMany({
          where: {
            ...
            ...buildAnalyticsSubmissionDateWhere(filters),
          },
          select: {
            assignmentId: true,
            status: true,
  ...
  for (const submission of submissions) {
    ...
    if (submission.grade) {
      ...
      if (submission.grade.gradedAt && submittedAt) {
        const diffMs = submission.grade.gradedAt.getTime() - submittedAt.getTime()
        if (diffMs >= 0) {
      ...
      const rubricItems = readRubricBreakdown(submission.grade.rubricBreakdown)
  ...
  const rubricAverages: AnalyticsRubricAverage[] = Array.from(rubricTotals.entries()).map(
    ([criterion, totals]) => ({
      criterion,
  ...
  return {
    ...
    averageTurnaroundDays: safeAverage(totalTurnaroundSum, totalTurnaroundCount),
