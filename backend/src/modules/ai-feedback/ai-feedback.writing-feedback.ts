...
export async function enqueueAiWritingFeedbackForSubmission(
  ...
  // Submission retries resume failed enqueueing, but never regenerate completed work.
  const existing = await findLatestAiFeedbackDraftBySubmission(context.submission.id)
  if (
    existing?.inputHash === sha256(context.promptInput) &&
    (existing.status !== 'failed' || existing.nextRetryAt)
  ) {
    return toWritingFeedbackResponse(existing)
  }
