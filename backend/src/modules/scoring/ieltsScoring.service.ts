...
import { AUTO_SCORE_TYPES, scoreIeltsSubmission } from './ieltsScoring.utils.js'

export async function autoScoreSubmission(submissionId: string) {
  const existingGrade = await prisma.grade.findUnique({
    where: { submissionId },
  })
  if (existingGrade && !existingGrade.deletedAt) {
