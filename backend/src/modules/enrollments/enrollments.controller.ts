...
export async function postEnrollment(req: Request, res: Response): Promise<void> {
  ...
  const enrollment = await createEnrollment(req.body, actor)
  res.status(201).json(enrollment)
}

export async function deleteEnrollmentById(req: Request, res: Response): Promise<void> {
  const actor = req.user
  ...
  await deleteEnrollment(req.params, actor)
  res.status(204).send()
}
