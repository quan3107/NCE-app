...
- **2026-09-20:** Reverified generic draft recovery against a real submissions-read failure in the disposable database. Restored SELECT grants without changing domain data; no backend application changes. Evidence: `docs/e2e-route-state-verification.md`.
- **2026-09-20:** Extended real-API acceptance across 33 protected routes using reversible read failures, empty fixtures, and database locks in the separate disposable database, then reran affected cases after frontend fixes. Restored domain data/grants and released all test locks; no backend application changes. X-02 PASS for the documented route-state matrix. Evidence: `docs/e2e-route-state-verification.md`.
- **2026-09-20:** Verified public CMS and session behavior against the actual API and a separate disposable PostgreSQL database. Real transport failure, database-lock failure, published-only reads, empty course data, and restoration were checked; no backend application changes. SHELL-04 passes. Evidence: `docs/e2e-public-recovery-verification.md`.
...
- **2026-09-20:** Corrected review-found generic draft loss during background resource recovery. Keep the form mounted with disabled controls and a guarded submit handler; preserve all local fields through error/Retry, loading, and empty states. Real-app submissions failure/recovery, ESLint, TypeScript, 208 component tests, 269 unit tests, and build pass. X-02 remains PASS within its documented scope.
- **2026-09-20:** Fixed the protected-route state failures found across all 33 route declarations: NCE editor now gates unavailable lesson/book/unit data, missing grading records offer Back, Rubrics and assignment creation show dependency failures with Retry, and six lists explain empty results. Real-app failure/recovery/empty reruns, ESLint, TypeScript, 207 component tests, 269 unit tests, and build pass. X-02 PASS for the documented matrix and limits in `docs/e2e-route-state-verification.md`. Inventory: 107 PASS, 10 FAIL, 1 BLOCKED, 7 DECISION-GATED.
- **2026-09-20:** Added query-backed Retry to Home/About/Contact. Main already resolved the historic authenticated About hang after scoped/deferred profile notifications. Real in-app browser checks pass for all three roles, public navigation/reload, account destinations, initials, sign-out, anonymous transitions, and public recovery states. ESLint, TypeScript, 269 unit tests, 200 component tests, and build pass. SHELL-04 PASS; subsequent protected-route fixes and verification also pass X-02.
