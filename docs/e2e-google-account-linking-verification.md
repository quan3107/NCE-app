...
- All 84 migrations applied; authoritative migration history and bidirectional Prisma schema comparisons passed. Linking-table privilege checks denied Data API and ordinary backend request roles. OpenAPI validation and the source-backed route/contract inventory passed.
...
The first CI run caught a missing leading index on the new account foreign key. A separate forward migration adds it without modifying the already-pushed migration. A fresh 84-migration replay and the complete schema-governance probe pass, including the indexed-foreign-key requirement.

## Trusted issuer compatibility follow-up — 2026-09-22

The review regression reproduced before the fix: same-subject sign-in failed in both directions when the stored and incoming issuers used different trusted Google spellings. A shared exact allowlist now canonicalizes new identities and linking proofs to `https://accounts.google.com`, accepts existing `accounts.google.com` identities and pending proofs, and rejects all other spellings. Linking checks both aliases for existing email ownership while retaining subject, account, deletion, and concurrency checks. No migration or rewrite of existing identities is required.

Fresh real Browser/API acceptance used a new disposable database and the real Google client, with provider fixtures disabled and no intercepted responses. Google emitted `https://accounts.google.com` on both callbacks, observed through temporary issuer-only diagnostics removed before committing. After password-confirmed linking, the disposable identity's stored issuer was changed to `accounts.google.com`; genuine Google sign-in then reached the original account's dashboard. The same existing password also reached that dashboard. Database reads confirmed one matching user, one identity on that original owner, and no remaining linking proof. Linking, success, and dashboard screenshots were visually inspected.

The reverse incoming issuer direction is deterministic coverage, not a claim about what the live provider emitted. Eight unit regressions cover both identity directions and untrusted issuers; 22 real PostgreSQL linking tests cover canonical proof writes, legacy pending proofs, both issuer spellings for conflicting email/subject owners, untrusted proof rejection, and existing lifecycle/concurrency protections. The full backend suite passed 1,100 tests; backend lint and build passed. Frontend source was unchanged. Follow-up logs are in `/tmp/nce-google-issuer`; disposable resources were removed after verification.
